...
def events_handler():
    ...
    if "challenge" in request_body_json:
        ...
        user_id = evt["user"]["id"]
        user_name = evt["user"]["profile"]["real_name_normalized"]
...
def build_message(user_id, user_name, status_text, status_emoji):
    ...
    if len(status_text) > 0:
        ...
        message = [{
                        ...
                        "text": {
                            "type": "mrkdwn", 
                            "text": user_name + " updated their status:"
                        }
                    },
                    {
                        "type": "section",
                        "block_id": "StatusUpdateSec1",
                        "text": {
                            "type": "mrkdwn", 
        ...
        message = [{
                        ...
                        "text": {
                            "type": "mrkdwn",
                            "text": user_name + " cleared their status"
                        }
            },
                                {
                        "type": "divider",
...
def post_update(attachments):
    data = {
        "token": api_token,
        "channel": channel_id,
        "blocks": json.dumps(attachments, separators=(',', ':')),
        "pretty": True
    }
    try:
